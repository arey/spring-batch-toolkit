...
public class RemoveSpringBatchHistoryTasklet implements Tasklet, InitializingBean {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
        ...
        LOG.info("Number of rows deleted from the {}STEP_EXECUTION_CONTEXT table: {}", tablePrefix, rowCount);
        totalCount += rowCount;

        rowCount = jdbcTemplate.update(getQuery(SQL_DELETE_BATCH_STEP_EXECUTION), date);
        LOG.info("Number of rows deleted from the {}STEP_EXECUTION table: {}", tablePrefix, rowCount);
        totalCount += rowCount;

        rowCount = jdbcTemplate.update(getQuery(SQL_DELETE_BATCH_JOB_EXECUTION_CONTEXT), date);
        LOG.info("Number of rows deleted from the {}JOB_EXECUTION_CONTEXT table: {}", tablePrefix, rowCount);
        totalCount += rowCount;

        rowCount = jdbcTemplate.update(getQuery(SQL_DELETE_BATCH_JOB_EXECUTION_PARAMS), date);
        LOG.info("Number of rows deleted from the {}JOB_EXECUTION_PARAMS table: {}", tablePrefix, rowCount);
        totalCount += rowCount;

        rowCount = jdbcTemplate.update(getQuery(SQL_DELETE_BATCH_JOB_EXECUTION), date);
        LOG.info("Number of rows deleted from the {}JOB_EXECUTION table: {}", tablePrefix, rowCount);
        totalCount += rowCount;

        rowCount = jdbcTemplate.update(getQuery(SQL_DELETE_BATCH_JOB_INSTANCE));
        LOG.info("Number of rows deleted from the {}JOB_INSTANCE table: {}", tablePrefix, rowCount);
        totalCount += rowCount;
